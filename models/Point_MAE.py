...
class Attention(nn.Module):
    ...
    def forward_interpolate(self, x):
        ...
        q = F.interpolate(q, size=d_reduced, mode="linear", align_corners=False)
        k = F.interpolate(k, size=d_reduced, mode="linear", align_corners=False)
        
        self.scale = d_reduced**-0.5
        q = q.reshape(B, H, N, d_reduced)
        k = k.reshape(B, H, N, d_reduced)        
