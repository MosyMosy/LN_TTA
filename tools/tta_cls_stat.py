...
class LayerNormWithClsTransform(nn.LayerNorm):
    ...
    def __init__(self,
        ...
        self.prototype_std = prototype_std
        self.prototype = prototype
        self.mode = mode  # "simple", "full", or "hybrid"
        self.eps_bar_s = eps_bar_s
        self.apply_in_train = apply_in_train

    def _m_s_star(self, dtype, device, y_like):
        if self.prototype is not None:
            p = torch.as_tensor(self.prototype, dtype=dtype, device=device)
            m = p.mean()
            s = p.float().pow(2).mean().sqrt().to(dtype)
        else:
            m = torch.as_tensor(self.prototype_mean,
                                dtype=dtype,
                                device=device)
            s = torch.as_tensor(self.prototype_std, dtype=dtype, device=device)
        return m, s

    def _bar_s(self, dtype, device):
        if self.elementwise_affine and self.weight is not None:
            return self.weight.to(
                dtype=dtype,
                device=device).pow(2).mean().sqrt().clamp_min(self.eps_bar_s)
        return torch.as_tensor(1.0, dtype=dtype, device=device)

    def _bar_m(self, dtype, device):
        if self.elementwise_affine and self.bias is not None:
            return self.bias.to(dtype=dtype, device=device).mean()
        return torch.as_tensor(0.0, dtype=dtype, device=device)

    def forward(self, x):
        y = super().forward(x)  # post-affine output
        if self.training and not self.apply_in_train:
            return y

        m_star, s_star = self._m_s_star(y.dtype, y.device, y)
        if self.mode == "simple":
            return y * s_star + m_star

        bar_s = self._bar_s(y.dtype, y.device)
        bar_m = self._bar_m(y.dtype, y.device)

        if self.mode == "full":
            scale = s_star / bar_s
            shift = m_star - scale * bar_m

        return y * scale + shift


def replace_layernorm_with_cls_versions(model,
                                        cls_norms_embedding,
                                        device=None,
                                        skip=2,
                                        args=None):
            ...
            if isinstance(child, nn.BatchNorm1d):
                ...
                if norm_idx >= skip:
                    new_module = LayerNormWithClsTransform(
                        ...
                        prototype_mean=cls_norms_embedding[norm_idx -
                                                           skip].mean().item(),
                        prototype_std=cls_norms_embedding[norm_idx -
                                                          skip].std().item(),
                        prototype=cls_norms_embedding[norm_idx - skip],
                        mode=args.cls_fixer_ln_mode,
                    )
...
def reset_model(args, config, source_model, cls_norms_embedding):
    ...
    if "cls-fixer" in args.cls_fixer_mode:
        replace_layernorm_with_cls_versions(model,
                                            cls_norms_embedding,
                                            args=args)
...
def eval(args, config, logger, source_model, result_file_path):
    ...
    for args.severity in level:
        for corr_id, args.corruption in enumerate(corruptions):
            ...
            print(f"number of test samples: {len(tta_loader.dataset)}")
            ...
            for idx, (data, labels) in enumerate(tta_loader):
                ...
                labels = torch.cat(
                    [labels for _ in range(args.batch_size_tta)], dim=0)
                ...
                if "source_only" in args.cls_fixer_mode:
                    ...
                    if args.batch_size_tta > 1:
                        ...
                        logits = logits.view(args.batch_size_tta, -1,
                                             logits.shape[1])[0]
                    if not args.online:
                        with torch.no_grad():
                            logits = base_model(points)
            ...
            print_log(f"\nFinal Accuracy ::: {args.corruption} ::: {acc:.2f}%",
                      logger)
            f_write.write(f"{acc:.3f}\n")
            f_write.flush()

            if corr_id == len(corruptions) - 1:
                write_final_stats(f_write, time_list, acc_list,
                                  result_file_path)
